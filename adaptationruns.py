...
def main():
  ...
  if options.parallel == "19":
      alpha = 8
  if options.parallel == "20":
      alpha = 9
  if options.parallel == "21":
      alpha = 10
  orders = [1,2]
  ...
  if options.grids:
    for alpha in alphas:
      ...
      mysystem("./SMOOTH %s.mesh %s.mesh < MYSMOOTH"%(options.case,options.case))
          ...
          for cfl in cfls:
              for ptiter in ptiters:
                  for refinelevel in refinelevels:
                          ...
                          result = mysystem("./EULER < MYSOLVER%s"%options.parallel)
                          ...
                          if result==0 and order==2:
                              ...
                              mysystem("./EULER < MYSOLVER%s"%options.parallel)
                              mysystem("rm -f *.dat")
                          ...
                          if result==1 and refinelevel < 2:
                              ...
                              mysystem("./SMOOTH %s %s < MYSUBDIV%s"%(fname,fname,options.parallel))
                              mysystem("rm -f *.dat")
                              ...
                              mysystem("./SMOOTH %s %s < MYSMOOTH1%s"%(fname,fname,options.parallel))
